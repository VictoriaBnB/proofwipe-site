...
Clear overwrites the whole drive using normal write commands. It works on any drive, and once it is done the old data cannot be recovered with ordinary software. On a spinning hard drive it is thorough. On solid-state drives it has a known limit, which is why the second method exists.
